...
::: callout-note
## Reveal code: click the {{< fa solid code >}} icon (scroll to the top)

This page reveals the observablejs `{ojs}` code-chunks. However, this page demonstrates using `{r}` and `{ojs}` together in the same quarto-document. You can scroll to the top and click the {{< fa solid code >}} icon to see all the code. Or click {{< fa brands github >}} in the navigation bar. Quarto makes it easy to include these features.
:::
...
Notice the crispness and clarity of the plot (↥↥↥ above.) Even when generating a non-interactive plot, [Observable Plot](https://observablehq.com/@observablehq/plot) shows well on the web!
:::
